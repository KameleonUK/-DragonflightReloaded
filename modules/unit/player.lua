DFRL:NewDefaults("Player", {
    ...
    nameSize = {11, "slider", {8, 20, 0.5}, nil, "text settings", 6, "Player name text font size", nil, nil},
    levelSize = {11, "slider", {8, 20, 0.5}, nil, "text settings", 7, "Player level text font size", nil, nil},
    healthSize = {15, "slider", {8, 20, 0.5}, nil, "text settings", 8, "Health text font size", nil, nil},
    manaSize = {9, "slider", {8, 20, 0.5}, nil, "text settings", 9, "Mana text font size", nil, nil},
...
DFRL:NewMod("Player", 1, function()
    ...
    function Setup:RestingGlow()
        ...
        Setup.restingOverlay = CreateFrame("Frame", playerRestingGlow, PlayerFrame)
    ...
    callbacks.combatGlow = function(value)
        ...
        if value then
            Setup.combatOverlay:SetScript("OnUpdate", function()
                ...
                local alpha = Setup.combatGlow.alphaMin +
                (Setup.combatGlow.alphaMax - Setup.combatGlow.alphaMin) * (0.5 + 0.5 * math.sin(progress * 2 * math.pi))
                Setup.combatOverlayTex:SetAlpha(alpha)
                DFRL.activeScripts["CombatGlowScript"] = true
            end)
        else
    ...
    callbacks.restingGlow = function(value)
        ...
        if value then
            Setup.restingOverlay:SetScript("OnUpdate", function()
                ...
                local alpha = Setup.restingGlow.alphaMin +
                (Setup.restingGlow.alphaMax - Setup.restingGlow.alphaMin) *
